...
Попробуем отправить любой "адекватный вопрос", но мы ничего не получим, так что попробуем полистать пэйлоады. Ну и попробуем использовать "сигналы" LDAP (некоторый ряд символов, который позволит понять, есть ли уязвимость). Перепробовав несколько вариантов, мы понимаем, что здесь есть уязвимость вида ")".
# 
# 
### `PHP - Serialization`
